...
void DecodificarSerial() {
  ...
  int posicionDato = Mensaje.indexOf('/');
  int posicionNL = Mensaje.length();
  String Dato = Mensaje.substring(0, posicionDato);
  int Valor = Mensaje.substring(posicionDato + 1, posicionNL).toInt();
  if (Dato.equals("X")) {
    ValX = Valor;
  }
